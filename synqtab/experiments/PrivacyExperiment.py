...
class PrivacyExperiment(Experiment):
    ...
    def _run(self) -> None:
        ...
        LOG.info(f"Entering the _run() function of Privacy Experiment {str(self)}")
        ...
        if len(training_df) > MAX_TRAINING_ROWS:
            ...
            LOG.info(
                f"Experiment {str(self)} will be skipped, because the dataset has {len(training_df)} rows."
            )
            PostgresClient.write_skipped_computation(
                computation_id=str(self),
                reason=f"More than {MAX_TRAINING_ROWS} rows ({len(training_df)}).",
            )
        ...
        if self.data_error:
            if self.data_error_rate:
                data_error_instance = self.data_error.get_class()(
                    row_fraction=self.data_error_rate
                )
                training_df, corrupted_rows, corrupted_cols = (
                    data_error_instance.corrupt(
                        data=training_df,
                ...
                LOG.info(
                    f"Data Corruption was completed successfully for experiment {str(self)}"
                )
                ...
                if len(corrupted_cols) == 0:
                    ...
                    LOG.info(
                        f"Experiment {str(self)} will be skipped, because no columns to corrupt were found."
                    )
                    LOG.info(
                        f"Experiment {str(self)}. Categorical: {self.dataset.categorcal_features}, All: {training_df.columns}, Error Applicability: {data_error_instance.data_error_applicability()}."
                    )
                    self._should_compute = False
                    PostgresClient.write_skipped_computation(
                        computation_id=str(self), reason="No columns to corrupt."
                    )
                    return
        ...
        LOG.info(
            f"Generation for experiment {str(self)} was completed in {elapsed_time} seconds."
        )
        ...
        LOG.info(
            f"Successfully wrote the synthetic data of experiment {str(self)} to MinIO '{self.minio_path()}'."
        )
        ...
        corrupted_rows = (
            corrupted_rows.tolist()
            if "numpy" in str(type(corrupted_rows))
            else corrupted_rows
        )
        corrupted_cols = (
            corrupted_cols.tolist()
            if "numpy" in str(type(corrupted_cols))
            else corrupted_cols
        )
        ...
        PostgresClient.write_experiment(
            ...
            error_rate=(
                str(int(self.data_error_rate * 100)) if self.data_error_rate else None
            ),
        ...
        LOG.info(
            f"Successfully wrote the metadata of experiment {str(self)} to Postgres."
        )
    ...
    def _publish_tasks(self) -> Self:
        ...
        from synqtab.mappings import (
            ...
            SINGULAR_EVALUATION_TARGETS,
            DUAL_EVALUATION_TARGETS,
        )
        ...
        for evaluation_method in self.evaluators:
            ...
            if evaluation_method in SINGULAR_EVALUATORS:
                ...
                raise ValueError(
                    f"Evaluation method {str(evaluation_method)} was not found in neither the singular nor dual evaluators."
                )
        ...
        LOG.info(
            f"Successfully published {published_tasks} and skipped {skipped_tasks} tasks for experiment {str(self)}"
        )
        return self
